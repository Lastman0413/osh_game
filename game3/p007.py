...
def main():
    ...
    # 맵 생성 (설계도 기준: 현관 좌상·주방 상중·거실 중앙·베란다 하중·동쪽 방1·욕실·방2)
    rooms = {}
    
    # 거실 (중앙 허브)
    rooms["living"] = IndependentRoom("거실", 14, 12, COLOR_FLOOR_LIVING)
    rooms["living"].add_door(7, 0, "kitchen", 6, 5, "주방으로")       # 북 - 주방
    rooms["living"].add_door(0, 4, "entrance", 3, 4, "현관으로")      # 서 - 현관(좌상)
    rooms["living"].add_door(7, 11, "balcony", 3, 0, "베란다로")      # 남 - 베란다(하중)
    rooms["living"].add_door(13, 2, "bedroom1", 0, 7, "침실1로")      # 동 위 - 방1
    rooms["living"].add_door(13, 5, "bathroom", 0, 4, "욕실로")       # 동 중 - 욕실
    rooms["living"].add_door(13, 8, "bedroom2", 0, 4, "침실2로")      # 동 아래 - 방2
    
    # 현관 (좌상단, 좁은 세로) → 오른쪽으로 거실
    rooms["entrance"] = IndependentRoom("현관", 4, 8, COLOR_FLOOR_LIVING)
    rooms["entrance"].add_door(3, 4, "living", 0, 4, "거실로")
    
    # 주방 (상단 중앙) → 아래로 거실
    rooms["kitchen"] = IndependentRoom("주방 및 식당", 12, 6, COLOR_FLOOR_KITCHEN)
    rooms["kitchen"].add_door(6, 5, "living", 7, 0, "거실로")
    
    # 베란다 (하단 중앙) → 위로 거실
    rooms["balcony"] = IndependentRoom("베란다", 6, 10, COLOR_FLOOR_BALCONY)
    rooms["balcony"].add_door(3, 0, "living", 7, 11, "거실로")
    
    # 방1 (우상단, 주방 오른쪽) → 왼쪽으로 거실, 서쪽 문 아래편
    rooms["bedroom1"] = IndependentRoom("침실1", 10, 10, COLOR_FLOOR_BEDROOM)
    rooms["bedroom1"].add_door(0, 7, "living", 13, 2, "거실로")
    
    # 욕실 (중앙 우측) → 왼쪽으로 거실
    rooms["bathroom"] = IndependentRoom("욕실", 6, 8, COLOR_FLOOR_BATH)
    rooms["bathroom"].add_door(0, 4, "living", 13, 5, "거실로")
    
    # 방2 (우하단) → 왼쪽으로 거실
    rooms["bedroom2"] = IndependentRoom("침실2", 8, 8, COLOR_FLOOR_BEDROOM)
    rooms["bedroom2"].add_door(0, 4, "living", 13, 8, "거실로")
    
    # 현재 맵 (거실에서 시작)
    current_room_name = "living"
