...
def render_game(screen, room, characters, cam_off):
    ...
    # 서쪽 벽 (Bottom-Left) - [수정된 부분]
    # 시작점: (origin_x, origin_y), 끝점: (origin_x, origin_y + th)
    bx, by = room.origin_x * TILE_SIZE, room.origin_y * TILE_SIZE
    ex, ey = room.origin_x * TILE_SIZE, (room.origin_y + room.th) * TILE_SIZE
    b_iso = to_iso(bx, by)
    e_iso = to_iso(ex, ey)
...
def main():
    ...
    pygame.display.set_caption("미니의 정교한 이소메트릭 룸 (벽 위치 변경)")
    clock = pygame.time.Clock()
    
    room = Room()
    # 캐릭터 시작 위치를 방 안쪽으로 조정
