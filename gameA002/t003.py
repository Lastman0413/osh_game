...
COLOR_WALL = (120, 100, 80)
COLOR_FLOOR = (230, 220, 200)
COLOR_OUTSIDE = (173, 216, 230)
...
class FamilyMember:
    ...
    def draw_dog(self, surface, cx, cy, swing, bobbing):
        body_y = cy - self.limb_len - bobbing
        body_w = self.body_w + 4
        body_h = self.body_h + 2

        pygame.draw.ellipse(surface, self.body_color, (cx - body_w//2, body_y - body_h, body_w, body_h))

        tail_wag = math.sin(self.walk_count * 2) * 5
        pygame.draw.line(surface, self.hair_color, (cx - body_w//2 + 2, body_y - body_h + 4), 
                        (cx - body_w//2 - 6 + tail_wag, body_y - body_h - 4), 3)

        leg_y = body_y
        leg_offset = swing * 0.8
        pygame.draw.line(surface, self.hair_color, (cx - 5, leg_y), (cx - 5 - leg_offset, leg_y + 8), 3)
        pygame.draw.line(surface, self.hair_color, (cx + 5, leg_y), (cx + 5 + leg_offset, leg_y + 8), 3)

        head_y = body_y - body_h + 6
        pygame.draw.circle(surface, self.head_color, (int(cx), int(head_y)), self.head_r + 2)

        if not self.look_dir.y < -0.1:
            ear_x = cx - self.look_dir.x * 3
            ear_y = head_y - 2
            pygame.draw.ellipse(surface, self.hair_color, (ear_x - 6, ear_y - 6, 8, 12))
            pygame.draw.ellipse(surface, self.hair_color, (ear_x + 2, ear_y - 6, 8, 12))

        eye_x = cx + self.look_dir.x * 2
        eye_y = head_y - 1
        pygame.draw.circle(surface, (255, 255, 255), (int(eye_x - 4), int(eye_y)), 4)
        pygame.draw.circle(surface, (255, 255, 255), (int(eye_x + 4), int(eye_y)), 4)
        pygame.draw.circle(surface, (30, 20, 10), (int(eye_x - 4), int(eye_y)), 2)
        pygame.draw.circle(surface, (30, 20, 10), (int(eye_x + 4), int(eye_y)), 2)

        nose_x = cx + self.look_dir.x * 5
        nose_y = head_y + 4
        pygame.draw.circle(surface, (20, 10, 10), (int(nose_x), int(nose_y)), 2)

        if self.is_controlled:
            pygame.draw.circle(surface, (255, 255, 0), (int(cx), int(head_y - self.head_r - 10)), 4)

class Room:
    def __init__(self, name, x, y, w, h, is_transition=False, next_room=None, theme_colors=None):
        self.name = name
        ...
        self.theme_colors = theme_colors or {"floor": (230, 220, 200), "floor_border": (200, 200, 190), "wall": (120, 100, 80), "wall_border": (100, 80, 60)}
    ...
    def pre_render(self, tile_size):
        ...
        for tx in range(floor["w"]):
            for ty in range(floor["h"]):
                ...
                pygame.draw.polygon(self.rendered_surface, self.theme_colors["floor"], pts)
                pygame.draw.polygon(self.rendered_surface, self.theme_colors["floor_border"], pts, 1)
        ...
        for wall in self.walls:
            ...
            if wall["type"] == "horizontal":
                ...
                pygame.draw.polygon(self.rendered_surface, self.theme_colors["wall"], wall_pts)
                pygame.draw.polygon(self.rendered_surface, self.theme_colors["wall_border"], wall_pts, 2)
...
class Game:
    ...
    def create_rooms(self):
        return [
            Room("현관", 50, 50, 30, 25, next_room=1,
                 theme_colors={"floor": (230, 220, 200), "floor_border": (180, 170, 150), "wall": (140, 120, 100), "wall_border": (120, 100, 80)}),
            Room("복도", 80, 50, 20, 25, next_room=2,
                 theme_colors={"floor": (220, 210, 190), "floor_border": (180, 170, 150), "wall": (130, 110, 90), "wall_border": (110, 90, 70)}),
            Room("거리1", 150, 50, 80, 60, next_room=3,
                 theme_colors={"floor": (200, 195, 180), "floor_border": (170, 165, 150), "wall": (160, 140, 120), "wall_border": (140, 120, 100)}),
            Room("산길", 50, 100, 80, 60, next_room=4,
                 theme_colors={"floor": (160, 180, 140), "floor_border": (130, 150, 110), "wall": (100, 130, 80), "wall_border": (80, 110, 60)}),
            Room("오솔길", 130, 110, 80, 60, next_room=5,
                 theme_colors={"floor": (170, 190, 150), "floor_border": (140, 160, 120), "wall": (90, 120, 70), "wall_border": (70, 100, 50)}),
            Room("시골길", 50, 160, 80, 60, next_room=6,
                 theme_colors={"floor": (210, 200, 160), "floor_border": (180, 170, 130), "wall": (140, 130, 100), "wall_border": (120, 110, 80)}),
            Room("할머니댁", 130, 170, 60, 50, is_transition=True,
                 theme_colors={"floor": (240, 230, 210), "floor_border": (200, 190, 170), "wall": (150, 120, 90), "wall_border": (130, 100, 70)}),
        ]
    ...
    def create_dialogs(self):
        return {
            ...
            3: [
                ("아빠", "아, 이 산길 좋다..."),
                ("엄마", "아..., 아빠!"),
                ("아빠", "괜찮아... 살짝 발을 삐끗했을 뿐이야"),
                ("시스템", "아빠가 다쳤습니다! 엄마의 치료가 필요합니다"),
            ],
            4: [
                ("엄마", "아빠, 괜찮아? 많이 다쳤어..."),
                ("아빠", "괜찮아... 이 오솔길에서 천천히 가자"),
                ("딸", "아빠! 힘내!"),
                ("강아지", "멍... (걱정)"),
            ],
            5: [
