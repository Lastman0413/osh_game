...
class Game:
    ...
    def create_rooms(self):
        return [
            Room("현관", 5, 5, 6, 5, next_room=1),
            Room("복도", 11, 5, 4, 5, next_room=2),
            Room("로비", 15, 5, 8, 6, next_room=3),
            Room("고미술관", 5, 10, 8, 6, next_room=4),
            Room("역사관", 13, 11, 8, 6, next_room=5),
            Room("자연관", 5, 16, 8, 6, next_room=6),
            Room("출구", 13, 17, 6, 5, is_transition=True),
        ]

    def create_family(self):
        return [
            FamilyMember(320, 340, "father", "아빠"),
            FamilyMember(280, 300, "mother", "엄마"),
            FamilyMember(240, 260, "daughter", "딸"),
            FamilyMember(260, 280, "dog", "강아지"),
        ]
    ...
    def update(self):
        if self.state == "playing":
            ...
            room_right_edge = (room.floor_area["x"] + room.floor_area["w"]) * TILE_SIZE - 30
            ...
            if leader.world_pos.x > room_right_edge and self.current_room_index < len(self.rooms) - 1:
                ...
                if next_room.is_transition:
                    ...
                    start_x = next_room.floor_area["x"] * TILE_SIZE + 50
                    ...
                    # 방에 들어서면 대화 트리거
                    if self.current_room_index in self.dialogs:
                        self.current_dialog = self.dialogs[self.current_room_index]
                        self.dialog_index = 0
                        self.state = "dialog"
    ...
    def draw_room(self, room):
        ...
        for tx in range(floor["w"]):
            for ty in range(floor["h"]):
                ...
                pts = [
                    (iso_x + self.cam_off[0], iso_y + self.cam_off[1]),
                    (iso_x + TILE_SIZE + self.cam_off[0], iso_y + TILE_SIZE/2 + self.cam_off[1]),
                    (iso_x + self.cam_off[0], iso_y + TILE_SIZE + self.cam_off[1]),
                    (iso_x - TILE_SIZE + self.cam_off[0], iso_y + TILE_SIZE/2 + self.cam_off[1])
                ]

                pygame.draw.polygon(self.screen, COLOR_FLOOR, pts)
                pygame.draw.polygon(self.screen, (180, 180, 180), pts, 1)
        ...
        for wall in room.walls:
            ...
            if wall["type"] == "horizontal":
                ...
                wall_height = 40
                wall_pts = [
                    (start_x + self.cam_off[0], start_y + self.cam_off[1]),
                    (end_x + self.cam_off[0], end_y + self.cam_off[1]),
                    (end_x + self.cam_off[0], end_y + wall_height + self.cam_off[1]),
                    (start_x + self.cam_off[0], start_y + wall_height + self.cam_off[1])
                ]
                pygame.draw.polygon(self.screen, COLOR_WALL, wall_pts)
                pygame.draw.polygon(self.screen, (80, 70, 60), wall_pts, 2)
    ...
    def draw_dialog(self):
        ...
        if speaker in ["아빠", "엄마", "딸", "강아지", "할머니"]:
            speaker_color = (100, 200, 255) if speaker == "아빠" else \
                           (255, 150, 150) if speaker == "엄마" else \
                           (150, 255, 150) if speaker == "딸" else \
                           (200, 200, 200) if speaker == "강아지" else (255, 200, 150)
        ...
        continue_text = self.font_small.render(" SPACE나 ENTER를 누르세요 ", True, (150, 150, 150))
