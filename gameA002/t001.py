...
class Game:
    ...
    def create_rooms(self):
        return [
            Room("현관", 5, 5, 8, 6, next_room=1),
            Room("로비", 13, 5, 10, 8, next_room=2),
            Room("고미술관", 5, 13, 10, 8, next_room=3),
            Room("역사관", 15, 13, 10, 8, next_room=4),
            Room("자연관", 10, 21, 10, 8, next_room=5),
            Room("출구", 20, 21, 6, 6, is_transition=True),
        ]
    ...
    def handle_events(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if self.state == "intro" or self.state == "dialog":
    ...
    def update(self):
        if self.state == "playing":
            ...
            leader = self.family[self.current_member]
            if leader.world_pos.x > 600 and self.current_room_index < len(self.rooms) - 1:
                self.current_room_index += 1
                self.family[self.current_member].world_pos.x = 100
