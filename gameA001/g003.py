...
class Survivor:
    def __init__(self, x, y):
        ...
        # 무기
        self.weapon = "fist"  # fist, bat, knife
        self.weapon_damage = {"fist": 10, "bat": 30, "knife": 25}
        self.attack_cooldown = 0
        self.attack_animation = 0  # 공격 애니메이션 카운터 (0 = 안함, >0 = 진행중)
    ...
    def update(self, house, zombies):
        ...
        for zombie in zombies:
            if zombie.alive and zombie.attack_animation > 0:
                if self.world_pos.distance_to(zombie.world_pos) < 35:
                    self.health -= 0.5
    ...
    def attack(self, zombies):
        """마우스 클릭으로 공격"""
        if self.attack_cooldown > 0 or not self.alive or self.attack_animation > 0:
            return False
        
        # 공격 애니메이션 시작
        self.attack_animation = 20  # 약 0.33초
        
        # 공격 범위 내 좀비 찾기
        attack_range = 60
        hit_zombie = False
        for zombie in zombies:
            if zombie.alive and self.world_pos.distance_to(zombie.world_pos) < attack_range:
                damage = self.weapon_damage[self.weapon]
                zombie.health -= damage
                zombie.stun_time = 15  # 경직 0.25초
                if zombie.health <= 0:
                    zombie.alive = False
                hit_zombie = True
                break  # 한 번에 하나만
        ...
        # 쿨다운 설정
        self.attack_cooldown = 30  # 0.5초
        return hit_zombie
    
    def pickup_item(self, item):
        self.inventory.append(item.type)  # 타입만 저장
        if item.type == "food":
    ...
    def draw(self, surface, cam_off):
        ...
        # 공격 모션 중이면 무기 휘두르기
        if self.attack_animation > 0:
            attack_progress = 1 - (self.attack_animation / 20)  # 0 -> 1
            swing_angle = attack_progress * 120 - 60  # -60도 -> 60도
            
            hand_x = cx + self.look_dir.x * 15
            hand_y = shoulder_y + 10
            
            if self.weapon == "bat":
                # 야구배트 휘두르기
                weapon_len = 35
                weapon_end_x = hand_x + math.cos(math.radians(swing_angle)) * weapon_len
                weapon_end_y = hand_y + math.sin(math.radians(swing_angle)) * weapon_len
                pygame.draw.line(surface, (150, 100, 50), (hand_x, hand_y), (weapon_end_x, weapon_end_y), 5)
                pygame.draw.circle(surface, (100, 70, 40), (int(weapon_end_x), int(weapon_end_y)), 6)
                ...
                weapon_end_x = hand_x + math.cos(math.radians(swing_angle)) * weapon_len
                weapon_end_y = hand_y + math.sin(math.radians(swing_angle)) * weapon_len
                pygame.draw.line(surface, (180, 180, 200), (hand_x, hand_y), (weapon_end_x, weapon_end_y), 4)
                pygame.draw.circle(surface, (100, 60, 40), (int(hand_x), int(hand_y)), 4)
            
            elif self.weapon == "fist":
                # 주먹 휘두르기
                fist_x = hand_x + math.cos(math.radians(swing_angle)) * 15
                fist_y = hand_y + math.sin(math.radians(swing_angle)) * 15
                pygame.draw.circle(surface, COLOR_LIMB, (int(fist_x), int(fist_y)), 6)
        else:
            # 평상시 팔
...
class Zombie:
    def __init__(self, x, y):
        self.world_pos = pygame.Vector2(x, y)
        self.walk_count = 0
        self.speed = 1.5  # 느리게 (일반인보다 느림)
        self.alive = True
        self.health = 50
        self.stun_time = 0  # 경직 시간
        self.attack_animation = 0  # 공격 애니메이션
        self.attack_cooldown = 0  # 공격 쿨다운
    ...
    def update(self, target_pos, house):
        ...
        # 가까우면 공격
        if distance < 40 and self.attack_cooldown == 0 and self.attack_animation == 0:
            self.attack_animation = 25  # 공격 모션 시작
            self.attack_cooldown = 60  # 1초 쿨다운
            return
    ...
    def draw(self, surface, cam_off):
        ...
        # 경직 중이면 빨간 섬광
        if self.stun_time > 0 and self.stun_time % 4 < 2:
            pygame.draw.circle(surface, (255, 100, 100), (int(cx), int(cy - 30)), 25, 2)
        ...
        if self.attack_animation > 0:
            # 공격 중 - 팔 앞으로
            attack_progress = 1 - (self.attack_animation / 25)
            arm_extend = attack_progress * 20
            pygame.draw.line(surface, (100, 120, 100), (cx - 10, cy - 30), (cx - 10 - arm_extend, cy - 20), 4)
            pygame.draw.line(surface, (100, 120, 100), (cx + 10, cy - 30), (cx + 10 + arm_extend, cy - 20), 4)
            
            # 공격 효과
            if attack_progress > 0.5:
                claw_x = cx + (arm_extend if arm_extend > 10 else -arm_extend)
                pygame.draw.line(surface, (200, 50, 50), (claw_x - 5, cy - 20), (claw_x + 5, cy - 25), 2)
                pygame.draw.line(surface, (200, 50, 50), (claw_x - 5, cy - 20), (claw_x + 5, cy - 15), 2)
        else:
            # 평상시 - 팔 늘어짐
        ...
        health_w = int((self.health / 50) * bar_w)
        pygame.draw.rect(surface, (100, 200, 100), (bar_x, bar_y, health_w, bar_h))
...
class Item:
    ...
    def draw(self, surface, cam_off):
        ...
        if self.type == "food":
            ...
            # 야구 배트
            pygame.draw.line(surface, (150, 100, 50), (cx - 2, cy - 10), (cx + 2, cy + 10), 4)
            pygame.draw.circle(surface, (100, 70, 40), (int(cx), int(cy - 12)), 4)
        elif self.type == "knife":
            # 칼
            pygame.draw.polygon(surface, (180, 180, 200), [
                (cx, cy - 10), (cx + 3, cy + 5), (cx - 3, cy + 5)
            ])
            pygame.draw.rect(surface, (100, 60, 40), (cx - 2, cy + 5, 4, 6))
...
def draw_ui(screen, survivor):
    ...
    # 무기
    weapon_names = {"fist": "주먹", "bat": "야구배트", "knife": "칼"}
    weapon_text = font.render(f"무기: {weapon_names[survivor.weapon]}", True, (255, 255, 255))
    screen.blit(weapon_text, (bar_x, bar_y + 65))
...
def main():
    ...
    items = [
        Item(350, 250, "food"),
        Item(450, 350, "medkit"),
        Item(250, 400, "bat"),      # 야구배트
        Item(520, 280, "knife")     # 칼
    ]
    ...
    while True:
        ...
        # 조작법
        help_text = font.render("WASD: 이동 | 좌클릭: 공격 | 우클릭: 줍기", True, (255, 255, 255))
        screen.blit(help_text, (WINDOW_WIDTH - 450, 20))
