    ...
    def draw(self, surface, cam_off):
        ...
        if self.role == "father":
            ...
            if not is_back:
                ...
                pygame.draw.ellipse(surface, self.hair_color, hair_rect)
                # 양갈래 (좌우)
                left_pigtail_x = cx - self.head_r - 3
                right_pigtail_x = cx + self.head_r - 3
                pigtail_y = head_y + 2
                # 왼쪽 양갈래
                pygame.draw.circle(surface, self.hair_color, (int(left_pigtail_x), int(pigtail_y)), 5)
                pygame.draw.circle(surface, self.ribbon_color, (int(left_pigtail_x), int(pigtail_y - 3)), 3)
                # 오른쪽 양갈래
                pygame.draw.circle(surface, self.hair_color, (int(right_pigtail_x), int(pigtail_y)), 5)
                pygame.draw.circle(surface, self.ribbon_color, (int(right_pigtail_x), int(pigtail_y - 3)), 3)
            else:
                # 뒤: 양갈래 뒤통수에서
                pygame.draw.circle(surface, self.hair_color, (int(cx), int(head_y - 2)), self.head_r - 1)
